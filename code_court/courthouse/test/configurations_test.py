...
class ConfigurationsTestCase(BaseTest):
    ...
    def _config_add(self, init_config_key):
        rv = self.app.post('/admin/configurations/add/', data={
            "key": init_config_key,
            "val": "True",
            "valType": "bool",
            "category": "admin",
        ...
        self.assertIn(init_config_key, page_config_keys, "Configuration was not added")
    ...
    def _config_edit(self, old_key, new_key):
        ...
        root = html.fromstring(rv.data)
        page_config_keys = [x.text for x in root.cssselect(".config_key")]
        self.assertIn(new_key, page_config_keys, "Configuration was not edited")

    def _config_del(self, key):
        config_id = model.Configuration.query.filter_by(key=key).one().id

        rv = self.app.get('/admin/configurations/del/{}'.format(config_id), follow_redirects=True)
        self.assertEqual(rv.status_code, 200, "Failed to delete configuration")

        rv = self.app.get('/admin/configurations/')
        root = html.fromstring(rv.data)
        page_config_keys = [x.text for x in root.cssselect(".config_key")]
        self.assertNotIn(key, page_config_keys, "Configuration was not deleted")
